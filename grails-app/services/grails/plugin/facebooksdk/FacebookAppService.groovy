...
class FacebookAppService {
    ...
    GrailsWebRequest getRequest() {
		return RequestContextHolder.requestAttributes as GrailsWebRequest
	}

    FacebookApp getFacebookApp() {
        // Get facebook app from request (created by FacebookSdkFilters)
        request.facebook.app
    }
	...
	Map exchangeAccessToken(String oldAccessToken) {
		...
		if (oldAccessToken) {
			...
			try {
                ...
                if (result) tokenData = result as Map
	...
	*/
	String getApplicationAccessToken(boolean oauthEnabled = false) {
		...
		if (oauthEnabled) {
			...
			def result = facebookGraphClient.fetchObject('oauth/access_token', parameters)
			if (result && result['access_token']) accessToken = result['access_token']
		} else {
	...
	* - no_user: the URL to go to if the user is not signed into facebook
	...
	String getLoginStatusURL(Map parameters = [:]) {
		if (!parameters['api_key']) parameters['api_key'] = facebookApp.id
		if (!parameters['no_session']) parameters['no_session'] = getCurrentURL()
		if (!parameters['no_user']) parameters['no_user'] = getCurrentURL()
		if (!parameters['ok_session']) parameters['ok_session'] = getCurrentURL()
		if (!parameters['session_version']) parameters['session_version'] =3
	...
	*/
	long getUserId() {
		if (!facebookAppRequestScope.hasData('userId')) {
			...
			if (signedRequest) {
				...
				if (accessToken && !(userId > 0 && accessToken == facebookAppPersistentScope.getData('accessToken'))) {
                    FacebookGraphClient facebookGraphClient = new FacebookGraphClient(accessToken)
					def result = facebookGraphClient.fetchObject('me', [fields: 'id'])
					if (result && result['id']) {
						userId = result['id'] as long
						if (facebookAppPersistentScope.getData('userId') != userId) {
							facebookAppPersistentScope.setData('userId', userId)
						}
					} else {
		...
		return facebookAppRequestScope.getData('userId', 0) as long
	}
	...
	private FacebookSignedRequest getSignedRequest() {
		if (!facebookAppRequestScope.hasData('signedRequest')) {
			if (request.getParameter('signed_request')) {
				...
				facebookAppRequestScope.setData('signedRequest', new FacebookSignedRequest(facebookApp.secret, request.getParameter('signed_request')))
			} else if (facebookAppCookieScope.hasCookie()) {
				// Cookie created by Facebook Connect Javascript SDK
                facebookAppRequestScope.setData('signedRequest', new FacebookSignedRequest(facebookApp.secret, facebookAppCookieScope.getValue()))
			}
		}
		return facebookAppRequestScope.getData('signedRequest', null) as FacebookSignedRequest
	}
